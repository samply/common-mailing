...
/**
 * This class is used to generate email texts from a main template and additional template files
 * that contain "Google Closure Templates". It passes the parameters that are set in an {@link
 * OutgoingEmail} to the templates.
 */
public class EmailBuilder {

  public static final String TEMPLATE_NAMESPACE = "samply.mailing";
  private static final String MAIN_MAIL_TEMPLATE_FILENAME =
      "/MainMailTemplate.soy";
  ...
  /**
   * Creates a new {@code EmailBuilder} instance. This constructor enforces that all the template
  ...
  /**
   * Creates a new {@code EmailBuilder} instance. This constructor enforces, that all template files
   * are in one folder. If the includeMainTemplate parameter is true, the main template is included
   * automatically.
   *
   * @param templateFolder      the folder containing all template files
  ...
  /**
   * Adds an additional Soy file that is used for processing. If such a file has the namespace
   * {@link #TEMPLATE_NAMESPACE}, it can override the templates "greeting", "maincontent" and
   * "footer" in the main mail template to customize the greeting or set the content and footer of
   * the mail. The parameter {@link OutgoingEmail#LOCALE_LEY} can be used for localization of the
   * text in all templates. The added file must be a
   * <a href="https://developers.google.com/closure/templates/">
   * "Google Closure Templates"</a>-template file.
   *
   * @param fileName            Name or relative path of the template file, relative to the template
   *                            folder
   * @param delegatePackageName Delegate package name if the Soy file is a delegate package (i. e.
   *                            there is {@code {delpackage delegatePackageName}} in its first
   *                            line)
   */
  ...
  /**
   * Constructs the text by using the main mail template and the additional templates supplied by
  ...
  public String getText(Map<String, String> parameters) {
    ...
    if (includeMainTemplate) {
      ...
      builder.add(new JarFileCharSource(mainTemplateJarPath),
  ...
  /**
   * A Character source for files packed in the jar. This is needed because files cannot be accessed
   * by path when they are inside a jar file.
   */
